...
interface ExecutionRejectModalProps {
  isOpen: boolean
...
const REQUIRED_REASON_MESSAGE = 'سبب رفض التنفيذ مطلوب.'

const ExecutionRejectModal = ({
  isOpen,
  isSubmitting = false,
  onConfirm,
  onClose,
}: ExecutionRejectModalProps) => {
    ...
    <div dir="rtl" className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          سيتم تحويل البلاغ إلى مراجعة بشرية لدى الإدارة بعد تأكيد السبب.
        </p>

        <label htmlFor="execution-reject-reason" className="mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">
          اكتب سبب الرفض هنا...
        </label>
        <textarea
          id="execution-reject-reason"
          value={reason}
...
export default ExecutionRejectModal
